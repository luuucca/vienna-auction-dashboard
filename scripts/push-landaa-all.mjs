/**
 * push-landaa-all.mjs
 * Translate Landaa listings via DeepL, push to Notion with photos & coords.
...
// ─── Main ─────────────────────────────────────────────────────────────────────
async function main() {
  const data = JSON.parse(fs.readFileSync('.tmp/landaa-all.json', 'utf8'));
  console.log(`Source: ${data.length} 套 Landaa 房源\n`);
  ...
  for (let i = 0; i < data.length; i++) {
    ...
    // Geocode ONLY if we have a real street. Landaa deliberately hides
    ...
    if (d.street) {
      ...
      console.log('  → 无街道 (Landaa fuzzy address)，跳过定位');
    }
